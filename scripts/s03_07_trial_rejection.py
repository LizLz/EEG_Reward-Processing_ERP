...
def find_artifacts(trials, maxMin, level, step, lowest):
    """
    Find artifacts/flag bad epochs given epoched data based on four criteria (see docstring of `trial_rejection_cust` for details).
    ...
    # CHECKPOINT 1: MaxMin, representing peak-to-peak noise
    ...
    # CHECKPOINT 2: Level, representing absolute amplitude level
    is_level_artifact =  np.max(np.abs(data), axis=2)  > level
    is_artifact |= is_level_artifact

    # CHEKPOINT 3: Step, representing sudden jumps between adjacent time points
    diff = np.diff(data, axis=2)
    is_step_artifact = np.any(diff > step, axis=2)
    is_artifact |= is_step_artifact

    # CHECKPOINT 4: lowest, representing dead channels with very low amplitude throughout the trial
    is_lowest_artifact = np.all(np.abs(data) < lowest, axis=2)
    is_artifact |= is_lowest_artifact

    return is_artifact

### ------------- Trial Rejection used in ica data before applying ica and all data before making erp------------------
def trial_rejection(eeg, stim_dict, maxMin=500e-6, level=500e-6, step=40e-6, lowest=0.1e-6, tmin=0, tmax=3, baseline=None):
    '''
