...
def binning(epochs, conditions_dict, bin_num=4):
    ...
    # Loop through all the task groups
    for group_name, markers in groups.items():
        
        # SAFETY CHECK: Only ask MNE for markers that actually exist in this subject's data
        available_markers = [m for m in markers if m in epochs.event_id]
        
        if not available_markers:
            print(f"Warning: No valid markers found for group {group_name} in this subject. Skipping.")
            continue # Skip to the next group to avoid crashing
            
        # Select epochs using the safe list
        current_epochs = epochs[available_markers]
...
def get_group_binned_rewp(n_bins, subjects, epoch_dict, binned_group_evokeds, learners_only=False):
    ...
    for s_idx, subj in enumerate(subjects):
        ...
        for i in range(n_bins):
            ...
            subj_results = rewp_calculation(subj_evokeds, epoch_dict)
            
            for cond in conditions:
                # 1. Check if the condition successfully calculated in rewp_calculation
                if cond in subj_results and 'mean' in subj_results[cond]:
                    rewp_per_subject_binned[cond][s_idx, i] = subj_results[cond]['mean']
                else:
                    # 2. If it's missing (0 trials), fill the array with NaN (Not a Number)
                    rewp_per_subject_binned[cond][s_idx, i] = np.nan
