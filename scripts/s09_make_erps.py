...
def get_trimmed_mean(epochs, proportiontocut):
    ...
    trimmed_evoked = mne.EvokedArray(
        ...
        epochs.info, 
        tmin=epochs.times[0],
        nave=len(epochs) * (1 - 2 * proportiontocut)
    )
...
def get_evoked(conditions_dict, epochs, proportiontocut=0.05):
    ...
    for name, marker in conditions_dict.items():
        # 1. Select the epochs safely
        try:
            # Try to grab the data
            epoch_cond = epochs[marker]
        except KeyError:
            print(f"Warning: Condition '{name}' ({marker}) missing from this subject. Skipping.")
            continue 

        # 2. Check if the condition exists but has 0 surviving trials
        if len(epoch_cond) == 0:
            print(f"Warning: No valid trials left for condition '{name}' after cleaning. Skipping.")
            continue
            
        # 3. Calculate your trimmed mean (or standard average)
